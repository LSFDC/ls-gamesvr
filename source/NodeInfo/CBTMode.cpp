...
CBTMode::CBTMode( Room *pCreator ) : Mode( pCreator )
{
...
void CBTMode::InitObjectGroupList()
{
...
void CBTMode::AddNewRecord( User *pUser )
{
	CatchRecord kRecord;
	kRecord.pUser = pUser;
...
void CBTMode::RemoveRecord( User *pUser, bool bRoomDestroy )
{
...
int CBTMode::GetUserKickVoteLimit( const ioHashString &szKickUserName )
{
...
ModeRecord* CBTMode::FindModeRecord( const ioHashString &rkName )
{
...
ModeRecord* CBTMode::FindModeRecord( User *pUser )
{
...
ModeRecord* CBTMode::FindModeRecord( int iIdx )
{
...
CatchRecord* CBTMode::FindCatchRecord( const ioHashString &rkName )
{
	return (CatchRecord*)FindModeRecord( rkName );
}

CatchRecord* CBTMode::FindCatchRecord( User *pUser )
{
	return (CatchRecord*)FindModeRecord( pUser );
}

void CBTMode::UpdateDieState( User *pDier )
{
...
void CBTMode::UpdateUserDieTime( User *pDier )
{
	CatchRecord *pDieRecord = FindCatchRecord( pDier );
	if( !pDieRecord ) return;
	if( !pDieRecord->bDieState ) return;
...
ModeType CBTMode::GetModeType() const
{
	return MT_CBT;
}

void CBTMode::GetModeInfo( SP2Packet &rkPacket )
{
...
void CBTMode::GetModeHistory( SP2Packet &rkPacket )
{
	int i = 0;
...
void CBTMode::GetCharModeInfo( SP2Packet &rkPacket, const ioHashString &rkName, bool bDieCheck )
{
...
int CBTMode::GetRecordCnt() const
{
	return m_vRecordList.size();
}

const char* CBTMode::GetModeINIFileName() const
{
	return "config/cbtmode.ini";
}

TeamType CBTMode::GetNextTeamType()
{
...
void CBTMode::CheckRoundEnd( bool bProcessCall )
{
	WinTeamType eWinTeam = WTT_DRAW;
...
void CBTMode::SetRoundEndInfo( WinTeamType eWinTeam )
{
	...
	{
		LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CBTMode::SetRoundEndInfo() m_iCurRound != m_vRoundHistory.size()" );
	}
	else
	{
		RoundHistory rh;
...
bool CBTMode::ProcessTCPPacket( User *pSend, SP2Packet &rkPacket )
{
...
void CBTMode::OnPrisonerEscape( User *pUser, SP2Packet &rkPacket )
{
	...
	{
		LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CBTMode::OnPrisonerEscape() - %s has not Team.",
...
void CBTMode::OnPrisonerDrop( User *pUser, SP2Packet &rkPacket )
{
...
void CBTMode::OnPrisonerMode( User *pUser, SP2Packet &rkPacket )
{
...
void CBTMode::UpdateRoundRecord()
{
	int iRecordCnt = GetRecordCnt();
	for( int i=0 ; i<iRecordCnt ; i++ )
	{
...
void CBTMode::OnEventSceneEnd( User *pSend, SP2Packet &rkPacket )
{
	bool bRoundChange;
	int iRoomIndex;
	...
	if( !pRecord )
	{
		LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CBTMode::OnEventSceneEnd - %s Not Exist Record",
...
int CBTMode::GetCurTeamUserCnt( TeamType eTeam )
{
	int iUserCnt = 0;
	int iRecordCnt = m_vRecordList.size();
	for( int i=0 ; i<iRecordCnt ; i++ )
	{
		...
		{
			LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CBTMode::GetCurTeamUserCnt() - User's ModeRecord is Not Exist(%d).", i );
		}
	}
...
bool CBTMode::CheckRoundJoin( User *pSend )
{
	...
	if( !pRecord )
	{
		LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CBTMode::CheckRoundJoin - %s Not Exist Record",
								 pSend->GetPublicID().c_str() );
		return false;
	}
