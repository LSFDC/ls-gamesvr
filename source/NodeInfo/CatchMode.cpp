...
CatchMode::CatchMode( Room *pCreator ) : Mode( pCreator )
{
...
void CatchMode::InitObjectGroupList()
{
...
void CatchMode::AddNewRecord( User *pUser )
{
	CatchRecord kRecord;
	kRecord.pUser = pUser;
...
void CatchMode::RemoveRecord( User *pUser, bool bRoomDestroy )
{
...
int CatchMode::GetUserKickVoteLimit( const ioHashString &szKickUserName )
{
...
ModeRecord* CatchMode::FindModeRecord( const ioHashString &rkName )
{
...
ModeRecord* CatchMode::FindModeRecord( User *pUser )
{
...
ModeRecord* CatchMode::FindModeRecord( int iIdx )
{
...
CatchRecord* CatchMode::FindCatchRecord( const ioHashString &rkName )
{
	return (CatchRecord*)FindModeRecord( rkName );
}

CatchRecord* CatchMode::FindCatchRecord( User *pUser )
{
	return (CatchRecord*)FindModeRecord( pUser );
}

void CatchMode::UpdateDieState( User *pDier )
{
...
void CatchMode::UpdateUserDieTime( User *pDier )
{
	CatchRecord *pDieRecord = FindCatchRecord( pDier );
	if( !pDieRecord ) return;
	if( !pDieRecord->bDieState ) return;
...
ModeType CatchMode::GetModeType() const
{
	return MT_CATCH;
}

void CatchMode::GetModeInfo( SP2Packet &rkPacket )
{
...
void CatchMode::GetModeHistory( SP2Packet &rkPacket )
{
	int i = 0;
...
void CatchMode::GetCharModeInfo( SP2Packet &rkPacket, const ioHashString &rkName, bool bDieCheck )
{
...
int CatchMode::GetRecordCnt() const
{
	return m_vRecordList.size();
}

const char* CatchMode::GetModeINIFileName() const
{
	return "config/catchmode.ini";
}

TeamType CatchMode::GetNextTeamType()
{
...
void CatchMode::CheckRoundEnd( bool bProcessCall )
{
	WinTeamType eWinTeam = WTT_DRAW;
...
void CatchMode::SetRoundEndInfo( WinTeamType eWinTeam )
{
...
bool CatchMode::ProcessTCPPacket( User *pSend, SP2Packet &rkPacket )
{
...
void CatchMode::OnPrisonerEscape( User *pUser, SP2Packet &rkPacket )
{
...
void CatchMode::OnPrisonerDrop( User *pUser, SP2Packet &rkPacket )
{
...
void CatchMode::OnPrisonerMode( User *pUser, SP2Packet &rkPacket )
{
...
void CatchMode::UpdateRoundRecord()
{
	int iRecordCnt = GetRecordCnt();
	for( int i=0 ; i<iRecordCnt ; i++ )
	{
...
void CatchMode::OnEventSceneEnd( User *pSend, SP2Packet &rkPacket )
{
	bool bRoundChange;
	int iRoomIndex;
	...
	if( !pRecord )
	{
		LOG.PrintTimeAndLog( LOG_DEBUG_LEVEL, "CatchMode::OnEventSceneEnd - %s Not Exist Record",
...
int CatchMode::GetCurTeamUserCnt( TeamType eTeam )
{
	int iUserCnt = 0;
	int iRecordCnt = m_vRecordList.size();
	for( int i=0 ; i<iRecordCnt ; i++ )
...
bool CatchMode::CheckRoundJoin( User *pSend )
{
